...
- نام کاربری ربات دیگر متغیر محیطی نیست. دکمه‌های «ورود با تلگرام» و «اتصال
  تلگرام» در اپلیکیشن کاربر را به https://t.me/<username>?start=... می‌فرستند
  و این نام کاربری از دیتابیس خوانده می‌شود: کانتینر ربات هر بار که بالا
  می‌آید آن را از روی توکن خودش ثبت می‌کند، پس معمولاً لازم نیست کاری بکنید.
  در اولین راه‌اندازی، ربات معمولاً پیش از تمام شدن migrate کانتینر وب بالا
  می‌آید؛ هر 10 ثانیه دوباره تلاش می‌کند، پس نام کاربری کمی بعد از پایان
  migrationها ثبت می‌شود. اگر بعد از 10 دقیقه تلاش را کنار بگذارد (یک خطا در
  لاگش)، کانتینر ربات را restart کنید. برای تنظیم دستی، از پنل ادمین
  اپلیکیشن Flutter (تب «تنظیمات»، بخش «ربات تلگرام») یا از Django admin
  (Telegram Bot Settings) استفاده کنید. مقداری که آنجا وارد شود همیشه اولویت
  دارد و ربات هرگز آن را بازنویسی نمی‌کند؛ اگر با رباتی که واقعاً در حال
  اجراست فرق داشته باشد، کانتینر ربات هشدار لاگ می‌کند و پنل ادمین آن را نشان
  می‌دهد. تا وقتی هیچ‌کدام از این دو وجود نداشته باشد، هر دو دکمه خطای 400
  برمی‌گردانند. اگر TELEGRAM_BOT_USERNAME هنوز در .env هست، migration شمارهٔ
  0006 اپ bot آن را فقط یک بار می‌خواند تا مقدار ادمین را پر کند، و بعد از
  اجرای آن migration می‌توانید حذفش کنید.
...
با هر فیش و هر لینک ورود خطا می‌دهد تا وقتی کانتینر وب migrate کند. برای bot
0005 تا 0006 (تنظیم نام کاربری ربات) ربات به کارش ادامه می‌دهد و تا وقتی
کانتینر وب migrate کند، هر 10 ثانیه دوباره برای ثبت نام کاربری‌اش تلاش می‌کند؛
فقط اگر بعد از 10 دقیقه تلاش را کنار بگذارد باید restart شود.
