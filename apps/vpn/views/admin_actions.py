    ...
    def post(self, request, payment_proof_id):
        ...
        try:
            ...
            if serializer.validated_data["approve"]:
                subscription = approve_payment_proof(proof, reviewed_by=request.user, admin_note=note)
                message = "Payment approved and applied on the VPN panel"
            else:
                subscription = reject_payment_proof(proof, reviewed_by=request.user, admin_note=note)
